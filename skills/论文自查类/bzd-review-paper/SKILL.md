...
11. Sum the ordinary deduction-based score. If it is below 20 and no non-compensable core failure applies, read and apply [references/low-score-safeguard.md](references/low-score-safeguard.md): independently rescore demonstrated work from zero, cap that bottom-up score at 35, and use the higher result. Then apply the AI-era post-score checks in `evidence-centered-scoring.md`: one 5-point abstract penalty when problem paragraphs omit what was being solved; a separate 5-point penalty for every materially distinct local equation block that lacks a nearby explanation of the equation's meaning/task connection or leaves necessary symbols undefined on first use; and a 10-point penalty for independently severe evidence-integrity failure. A symbol table never replaces local first-use explanation. Floor at zero and show page/formula evidence for every penalty.
...
After the ordinary ledger, show a separate `AI时代专项扣分与可读性门槛` table containing the abstract problem-description check, every local undefined-equation-symbol occurrence, any 10-point evidence-integrity penalty, the informed-outsider reading result, any 49.9 cap and the resulting raw-score arithmetic.

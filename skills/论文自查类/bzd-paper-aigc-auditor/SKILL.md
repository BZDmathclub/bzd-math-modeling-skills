...
description: 对数学建模竞赛论文进行两层AI痕迹审计——第一层9维检测(语言层面60%：连接词/排比/拔高词/被动句/段落规律/文本复杂度；事实层面40%：引文验证/数值一致性/术语一致性)，第二层模型合理性深度审查。输出分层HTML报告。
---

# BZD 数学建模论文AI痕迹审计工具 v2.1
## 融合nature-AIGC检测方法的增强版
...
**3.1 高频连接词检测**（维度1，权重12%）— 融合Humanizer-zh
   - 扫描全文，统计高频连接词密度（综上所述、此外、进一步、显然、具有特性等）
   - 计算: 密度 = (高频词总数) / (全文总句数)
   - 检查相同词的重复间隔（是否<5句）
   - 新增：检查"具有XX特性""相对而言""显然"等AI特化高频词
   - 风险判定：>25% → 0分(高风险)；20-25% → 20分；15-20% → 40分；<15% → 100分

**3.2 排比与同构句式识别**（维度2，权重7.2%）
   - 标记三项以上的排比句，检查排比后是否有量化数据
   - 检查排比项的来源说明（赛题给出/推导/约束）
   - 标记"动词+宾语"结构相同的连续句子，检查是否标注了章节/对象
   - 分级评分：L1全风险→0分，L2混合→50分，L3安全→100分

**3.3 拔高词与修饰词检测**（维度3，权重7.2%）— 5级分类
   - 识别拔高词并分级：L1(绝对化+无数据) / L2(相对化+泛泛对比) / ... / L5(完整定量+充分对比)
   - 检查拔高词后是否有量化证据（数字、对比、百分比）
   - 统计L1-L2级拔高词：>3个→0分，混合→30分，L3-L4为主→70分，L5为主→100分
   - 参考资源：[references/layer1-enhanced-detection.md](references/layer1-enhanced-detection.md) §2.3

**3.4 被动句占比与语态分析**（维度4，权重5.4%）
   - 抽取论文5个代表段落，统计被动句占比（正常范围10-20%）
   - 检查是否出现连续>3句都是被动语态的"生硬段落"
   - 被动句中是否都有明确的执行者
   - 占比评分：>30%→30分，25-30%→60分，15-25%→90分，<15%→100分

**3.5 段落结构规律性检测**（维度5，权重3.6%）
   - 抽取20段，统计段落开头词的重复率和段长分布
   - 检查开头方式是否多样化（"根据""为了""利用前问的"等）
   - 段落间是否有明确的逻辑承接（"由上可知""基于此"等）
   - 重复率>50%→20分，30-50%→60分，<20%→100分

**3.6 文本复杂度分析**（维度6，权重0.6%）
   - 计算词汇多样性TTR = 不同词数/总词数（目标>0.5）
   - 分析3-gram重复率（"本文采用""通过...方法"等）
   - 观察熵值和n-gram频率是否过于规律
   - TTR<0.4或3-gram>10%→30分，正常→100分
...
**4.1 引文验证**（维度7，权重15%）— 融合nature-detector的L1-L3检查
   ...
   - 评分：无法找到→0分(虚构)，部分不符→20分，全通过→100分

**4.2 数值一致性追踪**（维度8，权重15%）
   - 构建"参数溯源表"：列出所有关键参数(p=0.55, v=1m/s, Δt=0.5s等)
   - 追踪每个参数在全文的使用位置
   - 检查是否存在矛盾(P1=0.55, P10=0.54)或凭空出现
   - 验证参数来源是否清晰（赛题给出/前问推导/物理约束）
   - 评分：完全一致→100分，轻微波动(±1%)→70分，明显矛盾→0分

**4.3 术语一致性检测**（维度9，权重10%）
   - 构建"术语追踪表"：列出5-7个核心概念(直升机、运次数、多架次等)
   - 追踪各术语在全文的表述方式
   - 检查是否存在"一个东西多个名字"(遗传算法/进化策略/种群智能混用)
   - 评分：完全一致→100分，基本一致→80分，混用多种名字→30分

**5. 综合评分汇总**
   第一层得分 = 
   (维1×12% + 维2×7.2% + 维3×7.2% + 维4×5.4% + 维5×3.6% + 维6×0.6%) +
   (维7×15% + 维8×15% + 维9×10%)
